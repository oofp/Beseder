...
module Beseder.Base.Internal.STransFuncIx where
...
returnF a = MkFunc (returnT a)

invokeF  :: forall xs rs ex req name sp q m zs.
  ...
  , SplicC sp rs ex zs
  ) => Named name -> req -> STransFunc q m sp (InvokeAllFunc req name) ()
invokeF named req = 
  let strans :: STrans q m sp xs rs ex (InvokeAllFunc req name) ()
      strans = invoke named req
  in MkFunc strans
